...
static void rsa_generate_decryption_key(const mpz_t p, const mpz_t q, const mpz_t enc_key, mpz_t n, mpz_t dec_key) {
    ...
    // The encryption key is co-prime to lambda and 1 < enc < lambda(n);
    assert(mpz_cmp_ui(enc_key, 1) > 0);
...
void rsa_encrypt(const mpz_t plain, const mpz_t enc_key, const mpz_t n, mpz_t cipher) {
    // Let m represent the plaintext => the ciphertext is c = m^e mod(n);
    mpz_powm(cipher, plain, enc_key, n);
}

void rsa_decrypt(const mpz_t cipher, const mpz_t dec_key, const mpz_t n, mpz_t plain) {
...
void rsa_oaep_padding(const uint8_t* message, const size_t message_len, const uint8_t* parameter, const size_t param_len, uint8_t** em, const size_t em_len) {
    ...
    uint32_t* p_digest = NULL;
    uint8_t* p_hash = NULL;
    sha256(parameter, param_len, &p_digest);
    sha256_to_byte_array(p_digest, &p_hash);
    free(p_digest);
...
void rsa_mask_generation(const uint8_t* seed, const size_t seed_len, uint8_t* mask, uint32_t mask_len) {
    uint32_t counter = 0;
    uint32_t current_mask_len = 0;
    uint8_t bytes[4];
    uint8_t* temp = safe_malloc((seed_len + 4) * sizeof *temp);
    uint32_t* temp_digest = NULL;
    uint8_t* byte_digest = NULL;
